function [fit] = fitness(Pop, I) %Asumiendo valores positivos
    ...
    for ind=1:L(1) %Cogemos el siguiente individuo
        ...
        for alpha=0.2:0.2:2*pi
            x = round( (a*cos(deg2rad(alpha))+x0)*cos(deg2rad(theta)) + (b*sin(deg2rad(alpha))+y0)*sin(deg2rad(theta)));
            y = round((a*cos(deg2rad(alpha))+x0)*sin(deg2rad(theta))+(b*sin(deg2rad(alpha))+y0)*cos(deg2rad(theta)));
            elipse = [elipse; x y];
        end
